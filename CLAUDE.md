...
- `vms/saevm/saexec/state_processor.go` (+ its `ApplyTransaction` call site in `vms/saevm/saexec/execution.go`) - **Flare transaction application, era-split on Helicon.** `Execute` routes every tx through the exported `ApplyTransaction` selector: on configs with coreth extras and a scheduled Helicon, pre-Helicon block timestamps take legacy `dcore.ApplyTransaction` (coreth's full `TransitionDb` — daemon, fee burn, prioritised refunds, state connector, one-time governance/airdrop calls — bridged via `legacyChainContext`, whose nil `Engine()` is safe because the author is always passed explicitly); Helicon and later take `ApplyTransactionWithExtras`. The boundary matches coreth's own era rule (`errIsHeliconBlock` rejects `IsHelicon` blocks, both inclusive), and the live executor never sees pre-Helicon blocks, so the legacy branch fires only on tracing replays (`StateAtTransaction` and the block-tracer shadows in `vms/saevm/sae/rpc/trace_block.go`). `ApplyTransactionWithExtras` is a copy of libevm's unexported `core.applyTransaction` extended with the Flare fee settlement (`settleFees`: dEaD burn on Flare-family, coinbase burn on Songbird-family, prioritised-contract fee refund — the gas cap is checked against the **tx gas limit**, matching pre-Helicon `st.initialGas`) and the per-transaction daemon call. After `AtomicDaemonAndMint` it re-checks `evm.ExecutionInvalidated()`, reverts the statedb to a pre-`ApplyMessage` snapshot (voiding the whole transaction, like libevm's own check), and returns `execution invalidated: …`, preserving the pre-Helicon `TransitionDb` post-daemon check that libevm's `ApplyMessage`-time check cannot cover. Only these two mechanisms survive Helicon; state connector finalisation, governance settings, and airdrop/distribution updates were pre-Helicon one-time executions and are deliberately absent from the extras path (the legacy branch replays them for historical blocks). **Diff this file against libevm's `core/state_processor.go` on every libevm bump.** `settleFees` relies on `main.go`'s `RegisterAllLibEVMExtras` (coreth's `AfterExecutingTransaction` hook credits the base fee to the blackhole coinbase); on non-Flare chain IDs the behaviour is byte-for-byte upstream (no daemon call, fee settlement is a no-op), and configs without coreth extras or without a scheduled Helicon always take the extras path, which keeps the upstream saexec tests (which run without coreth's extras registration) working. Tests: `vms/saevm/saexec/state_processor_test.go` (incl. `TestApplyTransactionHeliconBoundary` for both sides of the fork).
- `vms/saevm/sae/rpc/trace_block.go` - **Flare block-level tracer shadows.** Shadows `debug_traceBlock`, `debug_traceBlockByNumber`, `debug_traceBlockByHash`, `debug_traceBlockFromFile`, and `debug_intermediateRoots` on the `tracerAPI` wrapper (same depth-0 shadowing pattern as its `TraceCall`; `tracerAPI` in `stateful.go` carries a `b *tracerBackend` field for this) so every transaction is replayed through the era-splitting `saexec.ApplyTransaction` instead of libevm's plain `core.ApplyMessage`, on both eras. Mirrors libevm's `eth/tracers/api.go` byte-for-byte otherwise (error strings, tracer construction, timeouts, `IntermediateRoots` partial-result semantics) — **diff against libevm's api.go on every libevm bump**. Deliberately sequential for JS tracers too: upstream's `traceBlockParallel` advances the shared state with untraced `ApplyMessage`, which would drop the Flare state changes — but the JS per-transaction error contract is preserved (a JS tracer construction/timeout/result error is recorded in that tx's `TxTraceResult.Error` and the block trace continues, aborting only on ctx cancellation or a genuine execution failure), with the JS tx traced on a `statedb.Copy()` and the shared state then advanced by an untraced era-aware apply; native tracers still abort the whole block on any per-tx error (regression test: `vms/saevm/sae/trace_block_flare_test.go` `TestDebugTraceBlockJSTracerPerTxErrors`). Blocks not served with an executed header (a raw `debug_traceBlock`/`debug_traceBlockFromFile` RLP block, or an `IntermediateRoots` bad block) carry SAE's worst-case base-fee *bound*, not the (lower) executed base fee transactions actually paid; replaying against the bound diverges Flare fee settlement, later txs' interim state, and the roots (and can spuriously reject a dynamic-fee tx with `ErrFeeCapTooLow`). `replayStateAtParent` → `executedBaseFeeBlock` normalises **both canonical and non-canonical** SAE blocks: it re-seals with the canonical block's stored `executedHeader` when the supplied block is that canonical block (`saexec.ApplyTransaction`'s `IsLegacyCorethBlock` false), and otherwise derives the base fee from the parent's executed gas clock + the block's timestamp via `saexec.DeriveExecutedBaseFee` (the same value `saexec.Execute` computes for a fresh block). Pre-Helicon coreth blocks (`IsLegacyCorethBlock` true) use the header base fee directly and are left unchanged. The full base-fee *divergence* only occurs in production (sustained load + execution backlog); the SAE test harness pins the base fee at its minimum (queue cap ≈2 blocks × ACP-176 scaling), so the derive path is exercised with a bogus above-cap header base fee (`TestDebugTraceBlockNonCanonicalDerivesBaseFee`, `TestIntermediateRootsBadBlockDerivesBaseFee` — both fail if the non-canonical block keeps its supplied header), and `saexec.IsLegacyCorethBlock` is unit-tested directly (`TestIsLegacyCorethBlock`). Before-block state (EIP-4788 beacon root + Flare before-block hooks) is applied by `replayStateAtParent` via `saexec.BeforeExecutingBlock` keyed off the **supplied** block, deliberately bypassing `tracerBackend.StateAtBlock`'s canonical-child hook — that hook keys off the canonical block at parent+1, wrong for a non-canonical `debug_traceBlock` RLP block or an `IntermediateRoots` bad block whose header (timestamp, parent-beacon-root) differs (regression test: `vms/saevm/sae/trace_block_flare_test.go` `TestDebugTraceBlockRLPBeforeBlockUsesSuppliedBlock`; `tracerBackend.StateAtBlock` remains for the still-promoted `traceChain`/`standardTrace`/bad-block endpoints). Tests: `vms/saevm/cchain/trace_flare_test.go` (`TestFlareTraceBlockReplaysFlareState` fails under the upstream loop by construction) plus the presae/`TestDebugTrace` parity suites.
