...
func addNodeFlags(fs *pflag.FlagSet) {
	...
	fs.Bool(NetworkAllowPrivateIPsKey, false, fmt.Sprintf("Allows the node to initiate outbound connection attempts to peers with private IPs. If the provided --%s is one of public neworks the default is false. Oterhwise, the default is true", NetworkNameKey))
