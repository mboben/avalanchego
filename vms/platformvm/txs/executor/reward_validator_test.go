...
func assertRewards(t testing.TB, chain state.Chain, stakerTx *txs.Tx, rewardTxID ids.ID, rewards wantReward) {
	...
	// The reward UTXOs of a RewardAutoRenewedValidatorTx occupy the first
	// indices (the tx itself has no outputs), in validation, delegatee order.
	// A zero-amount reward does not produce a UTXO. On Flare the potential
	// validation reward is always 0, so the validation UTXO is regularly
	// skipped.
	uStakerTx := stakerTx.Unsigned.(*txs.AddAutoRenewedValidatorTx)
	outputIndex := 0
	if rewards.validation > 0 {
		assertUTXO(t, chain, rewardTxID, outputIndex, rewards.validation, uStakerTx.ValidatorRewardsOwner.(*secp256k1fx.OutputOwners))
		outputIndex++
	}
	if rewards.delegatee > 0 {
		assertUTXO(t, chain, rewardTxID, outputIndex, rewards.delegatee, uStakerTx.DelegatorRewardsOwner.(*secp256k1fx.OutputOwners))
		outputIndex++
	}
	assertNoUTXO(t, chain, rewardTxID, outputIndex)
}
...
func assertRewardAutoRenewedValidator(
	...
	require.NoError(t, err)
	// Zero-amount rewards do not produce a UTXO; see assertRewards.
	wantAmounts := []uint64(nil)
	for _, amount := range []uint64{want.commitReward.validation, want.commitReward.delegatee} {
		if amount > 0 {
			wantAmounts = append(wantAmounts, amount)
		}
	}
	require.Len(t, rewardUTXOs, len(wantAmounts))
	for i, amount := range wantAmounts {
		require.Equal(t, amount, rewardUTXOs[i].Out.(*secp256k1fx.TransferOutput).Amount())
	}
}
...
func TestRewardDelegatorToAutoRenewedValidator(t *testing.T) {
	...
	// Verify the delegator reward UTXO on commit.
	...
	// Rewards are always 0 on Flare, so no delegator reward UTXO is created.
	require.Zero(t, wantDelegatorReward)
	delRewardOutputIndex := len(uDelegatorTx.Outputs()) + len(uDelegatorTx.Stake())
	assertNoUTXO(t, commitState, delegatorTx.ID(), delRewardOutputIndex)
	...
	// Verify no reward UTXOs are tracked via GetRewardUTXOs: rewards are
	// always 0 on Flare.
	rewardUTXOs, err := env.state.GetRewardUTXOs(delegatorTx.ID())
	require.NoError(t, err)
	require.Empty(t, rewardUTXOs)
}
