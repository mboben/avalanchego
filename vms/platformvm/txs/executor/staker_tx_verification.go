...
// verifyAddAutoRenewedValidatorTx carries out the validation for an AddAutoRenewedValidatorTx.
func verifyAddAutoRenewedValidatorTx(
	...
	if !backend.Config.UpgradeConfig.IsHeliconActivated(currentTimestamp) {
	...
	validatorRules, err := getValidatorRules(currentTimestamp, backend, chainState, tx.SubnetID())
	if err != nil {
		return err
	}
...
// verifySetAutoRenewedValidatorConfigTx carries out the validation for an SetAutoRenewedValidatorConfigTx.
func verifySetAutoRenewedValidatorConfigTx(
	...
	validatorRules, err := getValidatorRules(currentTimestamp, backend, chainState, autoRenewedStakerTx.SubnetID())
	if err != nil {
